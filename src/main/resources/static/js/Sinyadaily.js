...
function tiledata(data){
	for(var i =0;i<data.length;i++){
			if(data[i]["diff"]>0){
			$template = `<a href="./SinyaProduct?prodid=${data[i]["fk_prod_id"]}"><li class="SelectProduct  border border-secondary border-3 my-2" id=${data[i]["fk_prod_id"]} ><div class="text-danger mb-sm-1 mb-md-3 mb-5">${data[i]['prodname']} 上漲 ${data[i]['diff']} 元 現價 ${data[i]['price']} 元</div></li></a>`;
			test = $($template)
			test.appendTo("#result")	
			}else{
			$template = `<a href="./SinyaProduct?prodid=${data[i]["fk_prod_id"]}"><li class="SelectProduct  border border-secondary border-3 my-2" id=${data[i]["fk_prod_id"]} ><div class="text-success mb-sm-1 mb-md-3 mb-5">${data[i]['prodname']} 降價 ${Math.abs(data[i]['diff'])} 元 現價 ${data[i]['price']} 元</div></li></a>`;
			test = $($template)
			test.appendTo("#result")	
			}
		}
