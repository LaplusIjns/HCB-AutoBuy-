...
function tiledata(data){
	for(var i =0;i<data.length;i++){
			$template = `<a href="./Product?prodid=${data[i]["prod_id"]}"><li class="SelectProduct" id=${data[i]["prod_id"]} ><div class="text-info mb-sm-1 mb-md-3 mb-5">${data[i]['prodname']}</div></li></a>`;
			test = $($template)
			test.appendTo("#result")		
		}
	}
