...
public class RoutineJob {
    ...
    public void updateProdavailableToFalse() {
        ...
        List<Sinyamaintable> listSinyamaintable = sinyaMaintableService.getSinyamaintablePage(formData).getContent();
        List<Sinyamaintable> result = new ArrayList<>();
        log.info("共有 {} 筆資料須改更改為沒貨",listSinyamaintable.size());
        ...
        log.info("共有 {} 筆資料標記為沒貨",result2.size());
        for (Sinyamaintable sinyamaintable : result2) {
            log.info("異動產品編號: {} 產品名字: {}",sinyamaintable.getProdId(),sinyamaintable.getProdname());
        }
    }
    ...
    public void updateProdavailableToTrue() {
        ...
        List<Sinyamaintable> listSinyamaintable = sinyaMaintableService.getSinyamaintablePage(formData).getContent();
        List<Sinyamaintable> result = new ArrayList<>();
        log.info("共有 {} 筆資料須改更改為有貨",listSinyamaintable.size());
        ...
        List<Sinyamaintable> result2 = sinyaMaintableRepository.saveAllAndFlush(result);
        log.info("共有 {} 筆資料標記為有貨",result2.size());
        for (Sinyamaintable sinyamaintable : result2) {
            log.info("異動產品編號: {} 產品名字: {}",sinyamaintable.getProdId(),sinyamaintable.getProdname());
        }
    }
